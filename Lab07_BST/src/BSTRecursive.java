...
public class BSTRecursive {
	...
	public int numLeavesLessThan(BSTNode n, int v) {
		...
		if (n == null) {
			...
			count += numLeavesLessThan(n.left, v);
			count += numLeavesLessThan(n.right,v);
	 ...
	 * @throws Exception 
	 */
	public int nextOf(BSTNode n,int v) throws Exception {
		TreeIterator maxVal = (TreeIterator) findMax();
        if(maxVal.currentNode.data < v) {
            return v;
        } else {
            int returned = 0;
            ...
            if (n.data > v) {
                ...
                returned = nextOf(n.left, v);
                System.out.println("hello");
                System.out.println(returned);
            }else {
                returned = nextOf(n.right, v);
                System.out.println(returned);
            }
            if (returned == -1) {
                if (n.data == v) {
